...
{
    [Range(2,2048)] public int segmentsX = 256;
    [Range(2,2048)] public int segmentsZ = 256;

    void OnEnable()   => Build();
    void OnValidate()
        {
            // 👇 Solo reconstruir si no estamos en modo Play o si es seguro hacerlo
    #if UNITY_EDITOR
            // Evita el error de "SendMessage cannot be called during OnValidate"
    ...
    void Build()
    {
        ...
        for (int x=0; x<vx; x++)
        {
            float fx = x/(float)segmentsX;   // 0..1
            float fz = z/(float)segmentsZ;   // 0..1
            int i = z*vx + x;
        ...
        var m = new Mesh();
        m.indexFormat = (verts.Length>65535)
